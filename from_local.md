...
$$
BB(x) \neq \text{某种显式依赖 } BB(x-1) \text{ 的公式}
$$

定义 BB(x) 不需要先知道 BB(x-1)。但是，如果要严格证明 BB(x) 的精确值，通常必须分类所有 x 状态机器的停机行为。而 x-1 状态机器可以看成 x 状态机器的子集，因此证明 BB(x) 的过程中原则上也覆盖了 BB(x-1)。

结论：

$$
\boxed{BB(x) \text{ 定义上不依赖 } BB(x-1), \text{ 但证明 } BB(x) \text{ 往往包含 } BB(x-1) \text{ 的信息。}}
$$

这点很重要，因为它表明 BB 不是“楼梯式递推”，而是“每一层状态空间的整体审判”。
...
每一步只看当前状态和当前符号，只移动一格。规则极端局域。可是这样的局域规则可以产生 BB 级别的停机时间。

这说明：

$$
\boxed{\text{底层规则局域简单} \not\Rightarrow \text{整体行为有可计算的全局捷径。}}
$$

也就是说，BB 函数不是单纯的大数怪物，而是一个关于局域规则、整体行为和预测极限的边界案例。
...
$$
\text{空间局域性：影响不能瞬间跨越空间。}
$$

动力学里的局域性可以写成：

$$
\text{时间局域性：下一时刻由当前状态和局域规则决定。}
$$

BB 和图灵机提示我们，还可以定义一种计算论局域性：

$$
\text{计算步骤局域性：不能随意跳过因果计算链，直接获得全局结论。}
$$

所以“局域性”不应该只理解为坐标空间中的近邻关系。更广义地说：

$$
\boxed{\text{局域性是关于因果依赖链的结构。}}
$$
